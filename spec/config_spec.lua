...
describe("LuaRocks config tests #integration", function()
   ...
   describe("read config keys", function()
      ...
      it("reads a hash config key", function()
         local output = run.luarocks("config variables.LUA_DIR")
         assert.match("\".*\"", output)
      end)
   ...
   describe("write config keys", function()
      it("demands --scope", function()
         assert.is_false(run.luarocks_bool("config web_browser foo"))
      end)
